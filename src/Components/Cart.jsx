...
export default function Cart({ isCartOpen, setIsCartOpen }) {
  const { cartItems, removeFromCart, incrementQuantity, decrementQuantity, getSubtotal } = useCartStore();
                      ...
                      {cartItems.length === 0 ? (
                        ...
                        <ul role="list" className="-my-6 divide-y divide-gray-200">
                          {cartItems.map((product) => (
                            <li key={product.id || product._id || product.name + '-' + Math.random()} className="flex py-6">
                            <div className="size-24 shrink-0 overflow-hidden rounded-md border border-gray-200">
                                <img alt={product.imageAlt} src={product.imageUrl} className="size-full object-cover" />
                              </div>
